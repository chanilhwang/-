               ...
               bool show_sunk = true, bool show_all = false)
{
  ...
  while (boats_placed < num_boats)
  {
    ...
    do
    {
      ...
      boat.orientation = CS170::Utils::Random(0, 1)
                                 ? CS170::WarBoats::O_HORIZONTAL
                                 : CS170::WarBoats::O_VERTICAL;
    ...
    }while (bp == CS170::WarBoats::BP_REJECTED);
  ...
  while (num_boats > 0)
  {
    ...
    } while (sr == CS170::WarBoats::SR_DUPLICATE);

      // Sunk a boat
    if (sr == CS170::WarBoats::SR_SUNK)
      num_boats--;
    ...
    else if ( (sr == CS170::WarBoats::SR_SUNK) && show_sunk)
    {
      PrintShotResult(sr);
      PrintStats(ocean);
      Dump(ocean, 4, false);
...
int main(int argC, const char** argV)
{
  ...
  if(argC != ARG_NUM)
  {
    std::cout << "Usage: WarBoats <Test Number>" << std::endl;
    return -1;
  }
  int userValue = std::atoi(argV[1]);
  ...
  case 8:
    ...
    break;
  default:
    std::cout << "Test should be 0 - 8" << std::endl;
    break;
  }

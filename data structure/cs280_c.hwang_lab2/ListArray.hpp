...
template<typename T, int ArraySize>
inline void ListArray<T, ArraySize>::Sort_Selection()
{
	if (isSorted)
	{
		return;
	...
	std::pair<Node*, int> swap_index = std::make_pair(head, 0);
	std::pair<Node*, int> smallest_index = std::make_pair(head, 0);

	while (swap_index.first != tail || swap_index.second != tail->count)
	{
		for (auto curr = swap_index.first; curr != nullptr; curr = curr->pNext)
		{
			for (int i = (curr == swap_index.first) ? swap_index.second : 0; i < curr->count; i++)
			{
				if (smallest_index.first->values[smallest_index.second] > curr->values[i])
				{
					if (smallest_index.first == curr)
					{
						smallest_index.second = i;
					}
					else
					{
						smallest_index.first = curr;
						smallest_index.second = i;
					}
				}
			}
		...
		T temp = smallest_index.first->values[smallest_index.second];
		smallest_index.first->values[smallest_index.second] = swap_index.first->values[swap_index.second];
		swap_index.first->values[swap_index.second] = temp;

		if (swap_index.second == swap_index.first->count - 1 && swap_index.first->pNext != nullptr)
		{
			swap_index.first = swap_index.first->pNext;
			swap_index.second = 0;
		}
		else
		{
		...
		smallest_index.first = swap_index.first;
		smallest_index.second = swap_index.second;
	}
